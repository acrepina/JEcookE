...
public class DB {
	...
	public void addUser(UserModel user) {
		...
		try
		{
			...
			query =	connection.createStatement();
			int rs = query.executeUpdate( "INSERT INTO users (lastname, firstname, age, login, pwd) VALUES ('" + user.getSurname() + "', '" + user.getLastname() + "', '" + user.getAge() + "', '" + user.getLogin() + "', '" + user.getPwd() + "')");
			query.close	();
			connection.close();
		} catch (SQLException e) {
			e.printStackTrace();
